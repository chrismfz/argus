...
func (a *Anomaly) isTrained() bool {
	...
	if a.lastTrain.IsZero() {
		return false
	}
	// HBOS: trained when it has any training scores
	if a.hbos != nil && len(a.hbos.trainScores) == 0 {
		return false
	}
	// eHBOS: trained when it has any training scores
	if a.ehbos != nil && len(a.ehbos.trainScores) == 0 {
		return false
	}
	return true
}
...
func NewAnomaly(cfg AnomalyConfig, det Detector, store DetectionStore) *Anomaly {
	...
	if cfg.Label == "" {
		cfg.Label = "iforest_anomaly"
	}
	if cfg.RetrainEvery <= 0 {
		cfg.RetrainEvery = 5 * time.Minute
	}
	if cfg.BaselineMax <= 0 {
		cfg.BaselineMax = 50000
	}
	if cfg.MinScore <= 0 {
		cfg.MinScore = 0.70
	}
	// sensible defaults
	if cfg.RequireHBOSPercentile < 0 {
		cfg.RequireHBOSPercentile = 0
	}
...
func explainShape(v featureVector) []string {
	var r []string
	if v.TCPSYNRatio >= 0.95 {
		r = append(r, "syn-heavy")
	}
	switch {
	case v.UniqDstIPs <= 1:
	...
	if v.ICMPShare >= 0.30 {
		r = append(r, "icmp-heavy")
	}
	if v.PktsPerSec >= 500 {
		r = append(r, "high-pps")
	}
	if v.BytesPerSec >= 1e8 {
		r = append(r, "high-bps")
	} // ~100MB/s
	return r
}

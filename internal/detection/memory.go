...
func (m *MemoryLayer) initLogger() {
	m.once.Do(func() {
		...
		if err != nil {
			log.Fatalf("open risk log: %v", err)
		}
...
func (m *MemoryLayer) Update(ip string, score float64, feats featureVector) (state *riskState, reasons []string, shouldLog bool) {
	...
	if score >= 0.70 {
		reasons = append(reasons, "if>=0.70")
	}
	if s.Risk >= m.cfg.TauRisk {
		reasons = append(reasons, "risk>=tau")
	}
	if s.Debt >= m.cfg.DebtWarn {
		reasons = append(reasons, "debt>=warn")
	}
	if s.ConsecHigh >= m.cfg.ConsecHighWarn {
		reasons = append(reasons, "consec_high")
	}

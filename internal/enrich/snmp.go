...
func CollectSNMPStats() ([]SNMPStat, error) {
	...
	for index, name := range cache.names {
		...
		for _, v := range result.Variables {
			...
			switch {
			case strings.Contains(oid, ".10."):
				rxBytes = val
			case strings.Contains(oid, ".16."):
				txBytes = val
			case strings.Contains(oid, ".11."):
				rxPkts = val
			case strings.Contains(oid, ".17."):
				txPkts = val
			case strings.Contains(oid, ".7."):
				admin = uint8(val)
			case strings.Contains(oid, ".8."):
				oper = uint8(val)
			case strings.Contains(oid, ".3."):
				ifType = val
			}
		}

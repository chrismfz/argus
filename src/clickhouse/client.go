...
func InsertSNMPStats(records []SNMPStat) error {
	ctx := context.Background()
	batch, err := Global.PrepareBatch(ctx, `INSERT INTO snmp_stats (timestamp, if_index, if_name, rx_bytes, tx_bytes)`)
	if err != nil {
		return err
	}
	for _, r := range records {
		if err := batch.Append(r.Timestamp, r.IfIndex, r.IfName, r.RxBytes, r.TxBytes); err != nil {
			return err
		}
	}
	return batch.Send()

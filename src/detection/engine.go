...
func NewEngine(rules []DetectionRule, geo GeoIP, resolver DNSResolver, ifnames IFNameCache, asn uint32, prefixes []*net.IPNet, maxWin time.Duration) *Engine {
    return &Engine{
        rules:     rules,
        geoip:     geo,
        resolver:  resolver,
...
func (e *Engine) runDetection() {
    ...
    for _, rule := range e.rules {
        ...
        for _, act := range parseActions(rule.Action) {
            switch act {
            case "alert":
                LogDetection(rule, flows, e.geoip, e.resolver)
            case "clickhouse":

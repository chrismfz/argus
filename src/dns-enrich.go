...
func processPTRBatch(db *sql.DB, resolver *DNSResolver, table, ipField, ptrField string) {
	...
	if err != nil {
		log.Printf("[PTR][ERROR] SELECT failed for %s: %v", ipField, err)
		return
	}
	defer rows.Close()
	...
	for _, ip := range ipList {
		...
		if _, err := db.ExecContext(ctx, update, ptr, ip); err != nil {
			log.Printf("[PTR][ERROR] UPDATE failed for IP %s: %v", ip, err)
		} else {
			log.Printf("[PTR] Updated %s for %s: %s", ptrField, ip, ptr)
		}
	}
}
